...
const StationPage = () => {
    ...
    const {station, name, setName, location, setLocation, fetchStation, saveStation, sendStation, deleteStation, setStation} = useStation()
        ...
        <div className="station-page-wrapper">
            ...
            <div className="station-reactors-wrapper">
                ...
                <div className="station-info-container">
                    ...
                    {is_moderator && <span>Покупатель: {station.owner.name}</span> }
                    {is_moderator && <span>Модератор: {station.moderator.name}</span>}
                    {completed && <span>Год введения в строй: {station.opening_year > 0 ? station.opening_year : "Нет"}</span>}
                </div>


                <div className="inputs-container">

                    <CustomInput placeholder="Название АЭС" value={name} setValue={setName} disabled={!is_draft}  />
                    <CustomTextarea placeholder="Место расположения" value={location} setValue={setLocation} disabled={!is_draft}  />
